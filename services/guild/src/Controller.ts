...
class Controller {
  ...
  public async addMember(req: IRequest, res: Response) {
    try {
      ...
      const guild: any = await Guild.findOne({ owner: req.user.id });
      if (!guild) {
   ...
   * Change member role
   */
  public async changeRole(req: IRequest, res: Response) {
    try {
      let memberIndex;
      // Check if the guild exists
      const guild: any = await Guild.findOne({ owner: req.user.id });
   ...
   */
  public async modDKP(req: IRequest, res: Response) {
    try {
      let memberIndex;
      // Check if the guild exists
      const guild: any = await Guild.findOne({ owner: req.user.id });
   ...
   */
  public async removeMember(req: IRequest, res: Response) {
    try {
      let memberIndex;
      // Check if the guild exists
      const guild: any = await Guild.findOne({ owner: req.user.id });
   ...
   * Remove a guild
   */
  public async removeGuild(req: IRequest, res: Response) {
    try {
      // Check if a guild exists
      const guild = await Guild.findOne({ owner: req.user.id });
